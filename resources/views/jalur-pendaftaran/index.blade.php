...
@section('content')
    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        @if (\App\Support\Akses::boleh('jalur-pendaftaran', 'buat'))
            <a href="{{ route('jalur_pendaftaran.create') }}" class="rounded-lg bg-brand px-3 py-1.5 text-sm font-semibold text-white hover:bg-brand-dark">+ Tambah Jalur</a>
        @endif
    </div>
    ...
    <div class="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
                <tr><th class="px-4 py-3">Kode</th><th class="px-4 py-3">Nama</th><th class="px-4 py-3">T.A</th><th class="px-4 py-3">Keterangan</th><th class="px-4 py-3">Status</th><th class="px-4 py-3 text-right">Aksi</th></tr>
                <tr class="bg-white">
                    ...
                    <tr data-row class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-2">
                                @if (\App\Support\Akses::boleh('jalur-pendaftaran', 'ubah'))<a href="{{ route('jalur_pendaftaran.edit', $r->kode) }}" class="text-brand hover:underline">Ubah</a>@endif
                                @if (\App\Support\Akses::boleh('jalur-pendaftaran', 'hapus'))
                                    <form method="POST" action="{{ route('jalur_pendaftaran.destroy', $r->kode) }}" onsubmit="return confirm('Hapus jalur {{ $r->kode }}?')">@csrf @method('DELETE')<button class="text-red-600 hover:underline">Hapus</button></form>
                                @endif
                            </div>
                        </td>
                    </tr>
