...
@section('title', 'Kas Keluar ' . $rec->nomor_transaksi)
...
@section('content')
    <div class="mx-auto max-w-4xl">
        <div class="mb-4 flex items-center justify-between">
            ...
            @if ($rec->status === 'aktif' && \App\Support\Akses::boleh('cash-out', 'hapus'))
                <div x-data="{ open: false }" class="relative">
                    ...
                    <form x-show="open" x-cloak @click.outside="open = false" method="POST" action="{{ route('cash_out.void', $rec->kode_transaksi) }}"
        ...
        <div class="mb-4 grid gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm sm:grid-cols-4">
            ...
            <div><div class="text-xs text-gray-400">Rekening Sumber</div><div>{{ $rec->rekening?->nama_rekening ?? $rec->kode_rekening }}</div></div>
            ...
            <div><div class="text-xs text-gray-400">Vendor</div><div>{{ $rec->vendor?->nama_vendor ?? '—' }}</div></div>
        ...
        <div class="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
                    <tr><th class="px-4 py-3">Akun (Debit)</th><th class="px-4 py-3">Keterangan</th><th class="px-4 py-3 text-right">Nominal</th></tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="border-t-2 border-gray-200 bg-gray-50 font-semibold">
                        <td class="px-4 py-2.5" colspan="2">Total (Kredit {{ $rec->rekening?->nama_rekening ?? $rec->kode_rekening }})</td>
